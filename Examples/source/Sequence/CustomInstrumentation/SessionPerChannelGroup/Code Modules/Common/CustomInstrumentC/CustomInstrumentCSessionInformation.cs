...
{
    /// <summary>
    /// Defines an object that contains an individual <see cref="CustomInstrumentC"/> session and its related information.
    /// </summary>
    public class CustomInstrumentCSessionInformation : ISessionInformation
    {
        ...
        /// <summary>
        /// The <see cref="CustomInstrumentC"/> session.
        /// </summary>
        public CustomInstrumentC Session { get; }
        ...
        /// </summary>
        /// <param name="session">The <see cref="CustomInstrumentC"/> session.</param>
        public CustomInstrumentCSessionInformation(CustomInstrumentC session)
        {
            Session = session;
        ...
        public CustomInstrumentCSessionInformation(CustomInstrumentC session, IList<SitePinInfo> associatedSitePinList)
        {
            Session = session;
        ...
        public static void GenerateResourceDescriptorToSitePinDictionary(ISemiconductorModuleContext tsmContext)
        {
            ...
            foreach (var dutPin in dutPins)
            {
                foreach (var tsmSiteContext in tsmContext.GetSiteSemiconductorModuleContexts())
                {
                    ...
                    var resourceName = (session as CustomInstrumentC).ResourceName;
