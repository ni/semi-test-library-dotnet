...
The above mentioned methods throw `NISemiconductorTestException` in failure scenarios. The exception message varies depending on the specific issue encountered. Below are the possible scenarios and their corresponding exception messages:
...
// The above operation will throw an exception of NISemiconductorTestException following with message
...
var result= siteData.Abs();
// The above operation will throw exception of NISemiconductorTestException with an exception message of Math operations
...
var result = pinSiteData.Add(siteData);
// The above operation will throw an exception of NISemiconductorTestException as For Add operation,
...
var result = pinSiteData.Abs();
// Above operation with throw an exception of NISemiconductorTestException as Abs() does not support string type.
```