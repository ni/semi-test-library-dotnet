...
namespace NationalInstruments.Tests.SemiconductorTestLibrary.Unit.InstrumentAbstraction.DCPower
{
    ...
    public sealed class SourceTests : IDisposable
    {
        ...
        [InlineData(false)]
        [InlineData(true)]
        public void DifferentSMUDevices_ForceSequenceCurrentWithPerSiteSequence_CorrectValueAreSet(bool pinMapWithChannelGroup)
        {
            var sessionManager = Initialize(pinMapWithChannelGroup);
            var sessionsBundle = sessionManager.DCPower("VDD");
        ...
        [InlineData(false)]
        [InlineData(true)]
        public void DifferentSMUDevices_ForceSequenceCurrentWithPerPinPerSiteSequence_CorrectValueAreSet(bool pinMapWithChannelGroup)
        {
            var sessionManager = Initialize(pinMapWithChannelGroup);
            var sessionsBundle = sessionManager.DCPower("VDD");
...
#pragma warning restore xUnit1013 // Public method should be marked as test
        ...
        public void DifferentSMUDevices_ConfigureSequenceWithSiteData_CorrectVoltageMeasurementsFetched(bool pinMapWithChannelGroup)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Assert.Equal(expectedSequences.GetValue(i), results[i][0].VoltageMeasurements);
            }
        }
        ...
        public void DifferentSMUDevices_ConfigureSequenceWithPinSiteData_CorrectVoltageMeasurementsFetched(bool pinMapWithChannelGroup)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Assert.Equal(expectedSequences.GetValue(i, "VDD"), results[i].VoltageMeasurements);
            }
        }
