...
#pragma warning restore xUnit1013 // Public method should be marked as test
        ...
        public void DifferentSMUDevicesSetPerPinPerSiteTransientResponse_GetTransientResponse_ReturnsCorrectValue()
        {
            ...
            var sessionsBundle = sessionManager.DCPower(pinNames);
            var activeSites = GetActiveSites(sessionsBundle);
            var expectedTransientResponse = new PinSiteData<DCPowerSourceTransientResponse>(activeSites, new Dictionary<string, DCPowerSourceTransientResponse>()
            {
                [pinNames[0]] = DCPowerSourceTransientResponse.Normal,
                [pinNames[1]] = DCPowerSourceTransientResponse.Fast,
                [pinNames[2]] = DCPowerSourceTransientResponse.Slow
            });
            sessionsBundle.ConfigureSourceSettings(new PinSiteData<DCPowerSourceSettings>(activeSites, new Dictionary<string, DCPowerSourceSettings>()
            {
                [pinNames[0]] = new DCPowerSourceSettings { TransientResponse = expectedTransientResponse.GetValue(activeSites[0], pinNames[0]) },
                [pinNames[1]] = new DCPowerSourceSettings { TransientResponse = expectedTransientResponse.GetValue(activeSites[0], pinNames[1]) },
                [pinNames[2]] = new DCPowerSourceSettings { TransientResponse = expectedTransientResponse.GetValue(activeSites[0], pinNames[2]) }
            }));
