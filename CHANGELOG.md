...
- ### Changed
  ...
  - **Instrument Abstraction**
    ...
    - Fixed an issue where calling `MeasureAndPublishCurrent` and `MeasureAndPublishVoltage` returned a `PinSiteData` object that contains the primary pin name instead of the merged pin group name. This issue occurred when a merged pin group was present within a `DCPowerSessionsBundle` object. These methods now correctly return a `PinSiteData` object that contains the merged pin group name. This behavior is consistent with the behaviors of `MeasureCurrent` and `MeasureVoltage`.
